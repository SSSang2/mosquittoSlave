...
void subscribeFromMaster(struct client_config *s_cfg)
{
	...
	char *temp[] = {"mosquitto_sub","-h","163.180.117.97","-t","test","-p","10001",};
	int rc;
	...
	if(client_id_generate(&cfg, "Slave1")){
		return ;
	}

	//mosq = mosquitto_new(cfg.id, cfg.clean_session, &cfg);
	if(masterMosq == NULL)
		mosq = mosquitto_new("Slave1", cfg.clean_session, &cfg);
	else
		mosq = masterMosq;
...
void subscribeThread(struct client_config *s_cfg){
	...
	//printf("PORT 	: %d\n", s_cfg->port);
	//printf("TOPIC	: %s\n", s_cfg->topic);
		pthread_create(&tid_sub, NULL, (void *)subscribeFromMaster, (void *)s_cfg); 
}

void systemSubscribe(struct mosquitto_db *db){
		printf("=========== SYSTEM SUBSCRIBE =========\n");
		//mqtt3_sub_add(db, masterMosq, "NAMU/SYSTEM/PUBLISH/RELAY", 0, &db->subs);
}
